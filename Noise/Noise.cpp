/**
 * Credit goes to Stefan Gustavson, I would have never been
 * able to figure out how to write this code myself
 */
#include "Noise.hpp"
#include "../Globals.hpp"
using namespace std;
...
Grad grad4[]= { Grad(0,1,1,1), Grad(0,1,1,-1), Grad(0,1,-1,1), Grad(0,1,-1,-1),
...
uint8_t pp[] ={151,160,137,91,90,15,
...
Noise::Noise(uint32_t seed):rng(seed){
	vector<uint8_t> nums(256);
	for(int i=0;i<256;i++){
		nums.at(i) = i;
	}
	...
	int i = 0;
	while(nums.size() != 0){
		int index = rng.getRange(0,nums.size() - 1);
		p[i] = nums.at(index);
		//p[i] = pp[i];
...
double Noise::dot(Grad g,double x, double y, double z){
	return g.x*x + g.y*y + g.z*z;
}

double Noise::dot(Grad g, double x, double y, double z, double w){
...
double Noise::noise3D(double x, double y, double z){
...
double Noise::noise4D(double x, double y, double z, double w){
